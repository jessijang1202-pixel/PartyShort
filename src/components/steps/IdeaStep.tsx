...
import { RefreshCcw, ChevronRight, ChevronLeft, Lightbulb, Users, Heart, Zap, CheckCircle2 } from 'lucide-react';
...
export default function IdeaStep() {
  ...
  useEffect(() => {
    if (!session.ideas.length && session.planning) {
      doGenerate();
    }
  }, []);
  ...
  function handleSelect(idea: ContentIdea) {
    selectIdea(idea);
  }

  function handleNext() {
    if (!session.selectedIdea) {
      setError('아이디어를 선택해주세요.');
      return;
    }
    setStep('hooks');
  }
  ...
  return (
    <div className="slide-up space-y-6">
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="font-semibold text-slate-900 dark:text-white text-base">{idea.title}</h3>
                    {isSelected && <CheckCircle2 className="w-5 h-5 text-blue-600 shrink-0" />}
                  </div>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 mb-3">{idea.summary}</p>

                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                    <div className="flex items-start gap-1.5">
                      <Users className="w-3.5 h-3.5 text-indigo-500 shrink-0 mt-0.5" />
                      <div>
                        <p className="text-xs font-medium text-slate-500 dark:text-slate-400">타겟 앵글</p>
                        <p className="text-xs text-slate-700 dark:text-slate-300">{idea.targetAudienceAngle}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-1.5">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-700 dark:text-slate-300">{idea.emotionalAngle}</p>
                      </div>
                    </div>
                    <div className="flex items-start gap-1.5">
                      <Zap className="w-3.5 h-3.5 text-amber-500 shrink-0 mt-0.5" />
                      <div>
                        <p className="text-xs font-medium text-slate-500 dark:text-slate-400">공감 포인트</p>
                        <p className="text-xs text-slate-700 dark:text-slate-300">{idea.resonanceReason}</p>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div className="flex justify-between pt-2">
        <Button variant="secondary" leftIcon={<ChevronLeft className="w-4 h-4" />} onClick={() => setStep('planning')}>
          이전
        </Button>
