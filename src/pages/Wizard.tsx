...
import SimpleUploadStep from '../components/steps/SimpleUploadStep';
...
export default function Wizard() {
  const { session, settings, videoMode, simpleUploads, setStep, currentProjectId, setCurrentProjectId, loadProjectSession, resetSession } = useApp();
  ...
  const simpleSteps = ['hooks', 'script-split', 'veo-clip', 'slides', 'subtitle-narration'];
  const inSimpleRange = videoMode === 'simple' && simpleSteps.includes(currentStep);

  // Show upload screen first if user has media and hasn't uploaded yet
  const needsSimpleUpload = inSimpleRange
    && (session.planning?.hasVideos || session.planning?.hasPhotos)
    && simpleUploads === null;

  const StepComponent = needsSimpleUpload
    ? SimpleUploadStep
    : inSimpleRange
    ? AutoProcessStep
  ...
  function handleNew() {
    if (window.confirm('현재 작업을 초기화하고 새 프로젝트를 시작할까요?')) {
      resetSession();
    }
  }
        ...
        <div className="flex items-center justify-between gap-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5">
          <div className="flex items-center gap-2 min-w-0">
            {currentProjectId ? (
              ...
              <span className="text-xs text-slate-400 dark:text-slate-500">저장되지 않은 프로젝트</span>
            )}
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <button
              onClick={() => setDrawerOpen(true)}
              className="flex items-center gap-1.5 text-xs font-medium text-slate-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 px-2 py-1.5 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-950/30 transition-colors"
            >
